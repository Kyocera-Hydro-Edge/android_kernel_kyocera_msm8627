...
#ifdef CONFIG_ANDROID_RAM_CONSOLE
#define RAM_CONSOLE_PHYS_BASE_ADDR	0xFA900000
#define RAM_CONSOLE_PHYS_BUFF_SIZE	0x00080000

static struct persistent_ram_descriptor pram_descs[] = {
	{
		.name = "ram_console",
		.size = RAM_CONSOLE_PHYS_BUFF_SIZE,
	},
};
...
void __init kcj_add_persistent_ram( void )
{
	...
	pram->start = bank->start + bank->size - RAM_CONSOLE_PHYS_BUFF_SIZE;
#else
	pram->start = RAM_CONSOLE_PHYS_BASE_ADDR;
#endif
...
#define GPIO_CAMERA_PM8038		PM8038_GPIO_PM_TO_SYS(7)
#ifdef QUALCOMM_ORIGINAL_FEATURE
...
static struct i2c_board_info sensor_i2c_info[] __initdata = {
	{
		I2C_BOARD_INFO("accelerometer", 0x30 >> 1),
	},
	{
	...
	{
		I2C_BOARD_INFO("lps331ap_prs", 0xBA >> 1),
	},
	{
		I2C_BOARD_INFO("bmp18x", 0xEE >> 1),
	},
};
...
static void __init msm8930_cdp_init(void)
{
	...
	{
		printk( KERN_ERR "WCNSS will use internal XO(19.2MHz) \n");
...
int oem_get_power_on_status(void)
{
	void* entry = 0;
	int status = 0;
	size_t size = sizeof(status);

	entry = smem_get_entry(SMEM_POWER_ON_STATUS_INFO, &size);
...
{
	return oem_get_power_on_status() == PWR_ON_EVENT_USB_CHG;
}
