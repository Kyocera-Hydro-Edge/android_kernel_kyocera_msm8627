...
#ifndef FEATURE_KYOCERA_DATA_QCOM
static void diag_no_mutex_set_msg_mask(int rt_mask)
{
	int first_ssid, last_ssid, i;
	uint8_t *parse_ptr, *ptr = driver->msg_masks;
...
#ifndef FEATURE_KYOCERA_DATA_QCOM
void diag_no_mutex_toggle_event_mask(int toggle)
{
	uint8_t *ptr = driver->event_masks;
...
#ifndef FEATURE_KYOCERA_DATA_QCOM
static void diag_no_mutex_disable_log_mask(void)
{
	int i = 0;
	struct mask_info *parse_ptr = (struct mask_info *)(driver->log_masks);
...
#ifndef FEATURE_KYOCERA_DATA_QCOM
void diag_no_mutex_update_userspace_clients(unsigned int type)
{
	int i;
...
#ifndef FEATURE_KYOCERA_DATA_QCOM
void diag_no_mutex_send_log_mask_update(smd_channel_t *ch, int equip_id)
{
	void *buf = driver->buf_log_mask_update;
	int header_size = sizeof(struct diag_ctrl_log_mask);
	struct mask_info *ptr = (struct mask_info *)driver->log_masks;
...
#ifndef FEATURE_KYOCERA_DATA_QCOM
void diag_no_mutex_send_event_mask_update(smd_channel_t *ch, int num_bytes)
{
	void *buf = driver->buf_event_mask_update;
	int header_size = sizeof(struct diag_ctrl_event_mask);
	int wr_size = -ENOMEM, retry_count = 0, timer;
...
#ifndef FEATURE_KYOCERA_DATA_QCOM
void diag_no_mutex_send_msg_mask_update(smd_channel_t *ch, int updated_ssid_first,
						int updated_ssid_last, int proc)
{
	void *buf = driver->buf_msg_mask_update;
	int first, last, actual_last, size = -ENOMEM, retry_count = 0, timer;
...
static int diag_process_apps_pkt(unsigned char *buf, int len)
{
	...
	if (*buf == 0x73 && *(int *)(buf+4) == 3) {
		...
		diag_update_userspace_clients(LOG_MASKS_TYPE);
			...
			diag_no_mutex_disable_log_mask();
			diag_no_mutex_update_userspace_clients(LOG_MASKS_TYPE);
		}
#endif
#if defined(CONFIG_DIAG_OVER_USB)
		if (chk_apps_only()) {
			...
			*(int *)(driver->apps_rsp_buf + 4) = 0x0;
			...
			if (driver->ch_wcnss_cntl)
				...
				if (driver->ch_cntl)
					diag_no_mutex_send_log_mask_update(driver->ch_cntl,
								 ALL_EQUIP_ID);
				if (driver->chqdsp_cntl)
					diag_no_mutex_send_log_mask_update(driver->chqdsp_cntl,
								 ALL_EQUIP_ID);
				if (driver->ch_wcnss_cntl)
					diag_no_mutex_send_log_mask_update(driver->ch_wcnss_cntl,
								 ALL_EQUIP_ID);
            }
#endif
		...
		diag_update_userspace_clients(MSG_MASKS_TYPE);
			...
			diag_no_mutex_set_msg_mask(rt_mask);
			diag_no_mutex_update_userspace_clients(MSG_MASKS_TYPE);
		}
#endif
#if defined(CONFIG_DIAG_OVER_USB)
		if (chk_apps_only()) {
			...
			*(int *)(driver->apps_rsp_buf + 4) = rt_mask;
			...
			if (driver->ch_wcnss_cntl)
				...
				if (driver->ch_cntl)
					diag_no_mutex_send_msg_mask_update(driver->ch_cntl,
							 ALL_SSID, ALL_SSID, MODEM_PROC);
				if (driver->chqdsp_cntl)
					diag_no_mutex_send_msg_mask_update(driver->chqdsp_cntl,
							 ALL_SSID, ALL_SSID, QDSP_PROC);
				if (driver->ch_wcnss_cntl)
					diag_no_mutex_send_msg_mask_update(driver->ch_wcnss_cntl,
							 ALL_SSID, ALL_SSID, WCNSS_PROC);
            }
#endif
			ENCODE_RSP_AND_SEND(7);
		...
		diag_update_userspace_clients(EVENT_MASKS_TYPE);
			...
			diag_no_mutex_toggle_event_mask(*(buf+1));
			diag_no_mutex_update_userspace_clients(EVENT_MASKS_TYPE);
		}
#endif
#if defined(CONFIG_DIAG_OVER_USB)
		if (chk_apps_only()) {
			...
			if (driver->ch_wcnss_cntl)
				...
				driver->ch_wcnss_cntl, diag_event_num_bytes);
					...
					if (driver->ch_cntl)
						diag_no_mutex_send_event_mask_update(driver->ch_cntl,
								 diag_event_num_bytes);
					if (driver->chqdsp_cntl)
						diag_no_mutex_send_event_mask_update(driver->chqdsp_cntl,
								 diag_event_num_bytes);
					if (driver->ch_wcnss_cntl)
						diag_no_mutex_send_event_mask_update(
								 driver->ch_wcnss_cntl, diag_event_num_bytes);
            }
#endif
