...
const static uint8_t dnand_partition_guid[] = DNAND_DRV_DNAND_PARTITION_GUID;
...
static uint32_t search_dnand_partition(void)
{
    ...
    for( i = 0; i < entry_num / 4; i++ )
    {
        ...
        for( j = 0; j < 4; j++ )
        {
            if( !memcmp(dnand_partition_guid, lba.as.gpt_partition_entry[j].type_guid,
                        sizeof(dnand_partition_guid)) )
            {
