...
int msm_gpio_read_config_dir( unsigned gpio )
{
	return ((__raw_readl(GPIO_CONFIG(gpio)) >> 9) & 0x1);
}

int msm_gpio_read_config_drvstr( unsigned gpio )
{
	return ((__raw_readl(GPIO_CONFIG(gpio)) >> 6) & 0x7);
}

int msm_gpio_read_config_func( unsigned gpio )
{
	return ((__raw_readl(GPIO_CONFIG(gpio)) >> 2) & 0xf);
}

int msm_gpio_read_config_pull( unsigned gpio )
{
	return ((__raw_readl(GPIO_CONFIG(gpio)) >> 0) & 0x3);
}
