...
static struct pmic8xxx_pwrkey* the_pwrkey = 0;
int oem_is_off_charge(void);

void oem_notify_power_key_event(int code, int value)
{
	struct pmic8xxx_pwrkey *pwrkey = the_pwrkey;

	if (!pwrkey) {
		return;
	}

	input_report_key(pwrkey->pwr, code, 1);//press  we ignored value.
	input_report_key(pwrkey->pwr, code, 0);//release
...
static int __devinit pmic8xxx_pwrkey_probe(struct platform_device *pdev)
{
	...
	if (oem_is_off_charge()) {
		input_set_capability(pwr, EV_KEY, KEY_SLEEP);
		input_set_capability(pwr, EV_KEY, KEY_WAKEUP);
	}
