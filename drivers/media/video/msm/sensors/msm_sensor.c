...
int32_t msm_sensor_config(struct msm_sensor_ctrl_t *s_ctrl, void __user *argp)
{
	...
	switch (cdata.cfgtype) {
		...
		case CFG_SET_ISO:
			if (s_ctrl->func_tbl->
					sensor_set_manual_iso == NULL) {
				rc = -EFAULT;
				break;
			}
			rc = s_ctrl->func_tbl->
					sensor_set_manual_iso(s_ctrl,
											   cdata.cfg.iso_type);
			break;

		case CFG_SET_EXPOSURE_MODE:
			if (s_ctrl->func_tbl->sensor_set_ae_mode == NULL) {
				rc = -EFAULT;
				break;
			}
			rc = s_ctrl->func_tbl->
						sensor_set_ae_mode(s_ctrl,
									   cdata.cfg.ae_mode);
			break;
		...
		case CFG_SET_SCENE_MODE:
			if (s_ctrl->func_tbl->sensor_set_scene_mode == NULL) {
				rc = -EFAULT;
				break;
			}
			rc = s_ctrl->func_tbl->
						sensor_set_scene_mode(s_ctrl,
									   cdata.cfg.scene_mode);
			break;

		case CFG_GET_EXIF_INFO:
  			if (s_ctrl->func_tbl->
					sensor_get_exif_info == NULL ) {
				rc = -EFAULT;
				break;
			}
			rc = s_ctrl->func_tbl->
					sensor_get_exif_info(
						s_ctrl, cdata.cfg.exif_info_sensor);
			break;
		...
		case CFG_SET_FOCUS_MODE:
			...
			rc = s_ctrl->func_tbl->
						sensor_set_focus_mode(s_ctrl,
									   cdata.cfg.focus_mode);
			break;

       case CFG_SET_CANSEL_FOCUS:
			if (s_ctrl->func_tbl->sensor_cancel_focus == NULL) {
				rc = -EFAULT;
				break;
			}
			s_ctrl->func_tbl->sensor_cancel_focus();

			break;

		case CFG_GET_FLASH_INFO:
  			if (s_ctrl->func_tbl->
					sensor_get_flash_info == NULL ) {
				rc = -EFAULT;
				break;
			}
			rc = s_ctrl->func_tbl->
					sensor_get_flash_info(
						s_ctrl, cdata.cfg.flash_info_sensor);
			break;

		case CFG_GET_AE_AWB_STABLE_INFO:
  			if (s_ctrl->func_tbl->
					sensor_get_ae_awb_stable_info == NULL ) {
				rc = -EFAULT;
				break;
			}
			rc = s_ctrl->func_tbl->
					sensor_get_ae_awb_stable_info(
						s_ctrl, cdata.cfg.ae_awb_stable_info_sensor);
			break;
		case CFG_SET_FLASH_REG:
  			if (s_ctrl->func_tbl->
					sensor_set_flash_reg == NULL ) {
				rc = -EFAULT;
				break;
			}
			rc = s_ctrl->func_tbl->
					sensor_set_flash_reg(
						s_ctrl, cdata.cfg.flash_reg_mode);
			break;
#endif
#ifdef FEATURE_KYOCERA_MCAM
		case CFG_SET_ROI_AF_AREA:
			pr_err("%s CFG_SET_ROI_AF_AREA\n", __func__);
  			if (s_ctrl->func_tbl->
					sensor_set_roi_af_area_info == NULL ) {
				rc = -EFAULT;
				break;
			}
			rc = s_ctrl->func_tbl->
					sensor_set_roi_af_area_info(
						s_ctrl, &cdata.cfg.roi_af_area_info);
			break;
		case CFG_SET_ROI_AE_AREA:
			pr_err("%s CFG_SET_ROI_AE_AREA\n", __func__);
  			if (s_ctrl->func_tbl->
					sensor_set_roi_ae_area_info == NULL ) {
				rc = -EFAULT;
				break;
			}
			rc = s_ctrl->func_tbl->
					sensor_set_roi_ae_area_info(
						s_ctrl, &cdata.cfg.roi_ae_area_info);
			break;
#endif

#ifdef FEATURE_KYOCERA_MCAM
        case CFG_SET_MANUAL_EXPOSURE:
			CDBG("%s:CFG_SET_MANUAL_EXPOSURE\n", __func__);
			if (s_ctrl->func_tbl->
			sensor_write_manual_exposure == NULL) {
				rc = -EFAULT;
				break;
			}
			rc =
				s_ctrl->func_tbl->
				sensor_write_manual_exposure(
					s_ctrl,
					cdata.cfg.manual_exp.manual_exposure_index);
           break;
        case CFG_START_MANUAL_EXPOSURE:
			CDBG("%s:CFG_START_MANUAL_EXPOSURE\n", __func__);
			if (s_ctrl->func_tbl->
			sensor_start_manual_exposure == NULL) {
				rc = -EFAULT;
				break;
			}
			rc =
				s_ctrl->func_tbl->
				sensor_start_manual_exposure(s_ctrl);
           break;
        case CFG_STOP_MANUAL_EXPOSURE:
			CDBG("%s:CFG_STOP_MANUAL_EXPOSURE\n", __func__);
			if (s_ctrl->func_tbl->
			sensor_stop_manual_exposure == NULL) {
				rc = -EFAULT;
				break;
			}
			rc =
				s_ctrl->func_tbl->
				sensor_stop_manual_exposure(s_ctrl);
           break;
#endif
#ifdef FEATURE_KYOCERA_MCAM
        case CFG_START_CAF_TAKEPICTURE:
            CDBG("%s:CFG_START_CAF_TAKEPICTURE\n", __func__);
			if (s_ctrl->func_tbl->
			sensor_start_caf_takepicture == NULL) {
				rc = -EFAULT;
				break;
			}
			rc =
				s_ctrl->func_tbl->
				sensor_start_caf_takepicture(s_ctrl);
           break;
#endif
		default:
			rc = -EFAULT;
...
#ifdef FEATURE_KYOCERA_MCAM
void msm_sensor_vreg_ctl(const char* reg_name, bool onoff, int min_uV, int max_uV)
{
  static struct regulator *reg_ctrl;

  reg_ctrl = regulator_get(NULL, reg_name);
  if (IS_ERR(reg_ctrl)) {
...
int32_t msm_sensor_gpio_conf (struct msm_sensor_ctrl_t *s_ctrl, enum msm_sensor_gpio_conf gpio_en)
{
  int32_t rc = 0;
  struct msm_camera_sensor_info *data = s_ctrl->sensordata;
  CDBG("%s: %d\n", __func__, __LINE__);

  rc = msm_camera_request_gpio_table(data, (int)gpio_en);
  if (rc < 0) {
    pr_err("%s: request gpio failed\n", __func__);
  }
