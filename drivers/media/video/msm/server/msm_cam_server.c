...
struct msm_cam_v4l2_device *g_pcam = NULL;
#endif
...
#ifdef FEATURE_KYOCERA_MCAM
int msm_server_auto_focus_cb(void)
{
	int rc = 0;
	char send_val[] = {135,0};
	struct msm_ctrl_cmd ctrlcmd;
	...
	D("%s qid %d\n", __func__, g_pcam->server_queue_idx);
	idx = g_pcam->server_queue_idx;
	ctrlcmd.type	   = MSM_V4L2_SET_CTRL_CMD;
	ctrlcmd.timeout_ms = 10000;
	ctrlcmd.value = (char *)send_val;
	ctrlcmd.length = 1;
	ctrlcmd.vnode_id = g_pcam->vnode_id;
	ctrlcmd.queue_idx = g_pcam->server_queue_idx;
	ctrlcmd.config_ident = g_server_dev.config_info.config_dev_id[idx];
