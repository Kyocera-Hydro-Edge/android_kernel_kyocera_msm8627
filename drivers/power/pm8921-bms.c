...
#ifdef QUALCOMM_ORIGINAL_FEATURE
#else
static int total_delta_time_us = USEC_PER_SEC;
#endif
				...
				int delta_time_us, int new_soc, int prev_soc)
{
	...
	if (delta_time_us < USEC_PER_SEC)
		return prev_soc;
#else
	total_delta_time_us += delta_time_us;

	if (total_delta_time_us < USEC_PER_SEC) {
		return prev_soc;
	} else {
		total_delta_time_us = 0;
	}
#endif
...
static void oem_pm8921_bms_get_parameter(int *vbatt_uv, int *ibatt_ua, int *batt_temp)
{
	...
	pm_bms_read_output_data(the_chip, VSENSE_AVG, &vsense_raw);
	convert_vsense_to_uv(the_chip, vsense_raw, &vsense_uv);
	*ibatt_ua = vsense_uv * 1000 / (int)the_chip->r_sense_uohm;
...
void oem_pm8921_bms_calculate_cyclecorrection(void)
{
	...
	do {
		...
		case CHG_CYCLECORRECT_STATE_PARAM_OBTAINED2:
			...
			rbat_sf = interpolate_scalingfactor(
