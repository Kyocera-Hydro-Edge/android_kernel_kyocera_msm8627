...
#include <linux/oem_remote_notify.h>
...
#define DEFINE_LR(lr)	void* lr = __builtin_return_address(0)
#define debugk	printk
#define UPDATE_CHARGER_TASK_COUNT	10
static atomic_t oem_is_full = ATOMIC_INIT(0);
...
static struct smd_channel *smd_incall_ch = NULL;
static bool   smd_incall_opend = false;
#define	INCALL_SMD_RETRY_COUNT	20					/* times */
#define	INCALL_SMD_RETRY_DELAY	10					/* ms */
#define INCALL_SMD_PORT_NAME	"apr_incall_svc"
#define	INCALL_OPEN_RETRY_DELAY	2000				/* ms */

static void incall_queue_timer_cb(struct work_struct *);
static DECLARE_DELAYED_WORK(incall_timer, incall_queue_timer_cb);

static struct modem_status	modem_stat = {
	.stat_1x = INCALL_STAT_INVALID,
	.stat_evdo = EVDO_STAT_INVALID
};
...
static int pm_chg_get_fsm_state(struct pm8921_chg_chip *chip)
{
	...
	if (ret != fsm_state_last) {
		debugk("%s(%d)[0x%p]:fsm_state[%d->%d]\n", __func__, __LINE__, lr, fsm_state_last, ret);
		fsm_state_last = ret;
	}
...
#define SKIP_POWER_ON_COUNT		30
static uint32_t power_on_cnt = 0;
#endif
static int get_prop_batt_status(struct pm8921_chg_chip *chip)
{
	static int fsm_state_last = -1;
	static int batt_state_last = -1;
	...
	if (fsm_state == FSM_STATE_ON_CHG_HIGHI_1) {
		...
		else if (pm_chg_get_rt_status(chip, VBATDET_LOW_IRQ))
		{
			...
			pr_info("converted batt_state into POWER_SUPPLY_STATUS_CHARGING\n");
		}
		else
		{
			if (power_on_cnt < SKIP_POWER_ON_COUNT)
			{
				batt_state = POWER_SUPPLY_STATUS_CHARGING;
			}
		}
	...
	if (fsm_state_last != fsm_state) {
		debugk("%s(%d):fsm[%d->%d] batt[%d->%d] cnt[%d/%d]\n",
			__func__, __LINE__,
			fsm_state_last, fsm_state,
			batt_state_last, batt_state,
			power_on_cnt, SKIP_POWER_ON_COUNT);
	}
	fsm_state_last = fsm_state;
	batt_state_last = batt_state;
...
#ifdef QUALCOMM_ORIGINAL_FEATURE
#else
int oem_charger_enable(bool enable)
{
	int rc;
...
static void fast_chg_count_stop_worker_usb(struct work_struct *work)
{
	...
	if (oem_is_off_charge() && atomic_read(&oem_is_full)) {
		debugk("%s(%d):>>kernel_power_off\n", __func__, __LINE__);
		kernel_power_off();/* this must be called here. */
	}
}
#endif
...
int oem_pm8921_disable_source_current(bool disable)
{
	...
	if (disable) {
		...
		if(oem_iusbmax_current == OEM_IUSBMAX_NOT_SET) {
			...
			if (oem_iusbmax_current > USB_WALL_THRESHOLD_MA) {
				printk("%s usb_target_ma=%d oem_iusbmax_current=%d\n",
					__func__,usb_target_ma,oem_iusbmax_current);
...
static irqreturn_t bat_temp_ok_irq_handler(int irq, void *data)
{
...
#else
	{
		...
		if (the_chip->dc_present || the_chip->usb_present || oem_stand_detect)
		{
			...
			pr_err("power supply from charger by hard detection\n");
		}
	}
#endif
...
#else /* QUALCOMM_ORIGINAL_FEATURE */
#define VDD_MAX_INCREASE_MV	10 /* #195543 */
#endif/* QUALCOMM_ORIGINAL_FEATURE */
...
#define VBAT_ERROR_ELIMINATION	100
			...
			int vbat_batt_terminal_uv, int ichg_meas_ma)
{
	...
	if (!is_ext_charging(chip)) {
		...
		if (vbat_batt_terminal_uv / 1000 < vbat_intended - VBAT_ERROR_ELIMINATION) {
...
static void oem_battery_temp_set_status(int temp)
{
	...
	int16_t chg_batt_tmp_off_hot_work  = (50 * 10);
	int16_t chg_batt_tmp_on_hot_work   = (48 * 10);
	int16_t chg_batt_tmp_off_cold_work = (-20);
	int16_t chg_batt_tmp_on_cold_work  = 0;
	...
	case OEM_STATUS_NORMAL:
		if (chg_batt_tmp_off_cold_work >= temp) {
			battery_temp_status = OEM_STATUS_STOP;
		} else if (chg_batt_tmp_off_hot_work <= temp) {
			battery_temp_status = OEM_STATUS_STOP;
		} else if (wait_chg_on_work <= temp) {
	...
	case OEM_STATUS_STOP:
		if (chg_batt_tmp_on_cold_work > temp) {
			battery_temp_status = OEM_STATUS_STOP;
		} else if ((oem_param_charger.wait_chg_off * 10) >= temp) {
			battery_temp_status = OEM_STATUS_NORMAL;
		} else if (chg_batt_tmp_on_hot_work >= temp) {
			battery_temp_status = OEM_STATUS_LIMIT;
		} else {
			battery_temp_status = OEM_STATUS_STOP;
		}
...
static int oem_charger_limit_control_check(int enable)
{
	if(1 == enable) {
		if (OEM_STATUS_LIMIT != oem_charge_status) {
			...
			pr_info("limit chg start camera=%d substrate=%d battery=%d\n",
				camera_therm_status,substrate_therm_status,battery_temp_status);
		}
	} else if (0 == enable) {
		if (OEM_STATUS_LIMIT == oem_charge_status) {
			cancel_delayed_work_sync(&oem_charger_work);
			oem_pm8921_disable_source_current(false);
			pr_info("limit chg stop camera=%d substrate=%d battery=%d vbatt=%d\n",
				camera_therm_status,substrate_therm_status,battery_temp_status,vbatt_status);
		}
	} else {
...
{
	struct modem_status stat;
	stat = oem_remote_notify_get_modem_status();
	if (((the_chip->usb_present || the_chip->dc_present)) &&
		...
		if (((OEM_STATUS_STOP == camera_therm_status) &&
			...
			(OEM_STATUS_STOP == battery_temp_status)) {
			if (OEM_STATUS_STOP != oem_charge_status) {
				cancel_delayed_work_sync(&oem_charger_work);
				oem_pm8921_disable_source_current(false);
				oem_charger_enable(false);
				oem_charge_status = OEM_STATUS_STOP;
				pr_err("power supply from charger by soft detection\n");
			}
			...
			if ((((OEM_STATUS_LIMIT <= camera_therm_status) &&
			      ...
			      ((INCALL_STAT_TALK == stat.stat_1x) || (EVDO_STAT_ON == stat.stat_evdo))) {
...
static int oem_auto_charger_ctrl_cmd(const char *val, struct kernel_param *kp)
{
	...
	if ((0 == oem_auto_charger_ctrl) || (1 == oem_auto_charger_ctrl)) {
		ret = oem_charger_enable(!oem_auto_charger_ctrl);
	} else {
...
#else
	safety_time = oem_param_charger.time_chg_pm;
#endif
...
#define CHG_DNAND_PARAM_SIZE           160
#define CHG_DNAND_SHARE_SIZE           8
#define CHG_DNAND_SHARE_POINT          CHG_DNAND_PARAM_SIZE - CHG_DNAND_SHARE_SIZE
#define CHG_DNAND_CHG_DIS_POINT                CHG_DNAND_SHARE_POINT + 1
#define CHG_DNAND_CHG_DIS_SIZE         1

static struct smd_channel *chg_usb_dis_smd_ch = NULL;
static bool chg_usb_dis_is_open = false;
static uint8_t pbuf = 0;
static bool dnand_write_req = false;

static void oem_usb_chg_dis_notify(void *priv, unsigned event)
{
	int len = 0;
	...
	{
		case SMD_EVENT_DATA:
			len = smd_read_avail(chg_usb_dis_smd_ch);
			if (len != CHG_DNAND_CHG_DIS_SIZE)
			{
				printk("%s error bad length=%d \n", __func__, len);
				return;
			}
			smd_read_from_cb(chg_usb_dis_smd_ch, &pbuf, len);
			dnand_write_req = true;
			printk("%s data=%d len=%d \n", __func__, (int)pbuf, len);
			break;

		case SMD_EVENT_OPEN:
			chg_usb_dis_is_open = true;
			printk("%s open \n",__func__);
			break;

		case SMD_EVENT_CLOSE:
			chg_usb_dis_is_open = false;
			printk("%s close \n",__func__);
			break;
...
static void oem_chg_usb_dis_smd_control(void)
{
	int rc = 0;
	...
	if (!chg_usb_dis_is_open)
	{
		rc = smd_open("CHG_USB_DIS",
			&chg_usb_dis_smd_ch,
			NULL,
			oem_usb_chg_dis_notify);
		printk("%s smd_open=%d \n", __func__, rc);
	}
	...
	if (dnand_write_req)
	{
		kdnand_id_write( CHG_DNAND_CMD,
			CHG_DNAND_CHG_DIS_POINT,
			&pbuf,
			CHG_DNAND_CHG_DIS_SIZE);
		dnand_write_req = false;
		printk("%s dnand write=%d \n", __func__, (int)pbuf);
	}
}

static struct smd_channel *smd_ch = NULL;
static bool is_open = false;
static int send_connect_state = POWER_SUPPLY_OEM_CONNECT_NONE;

static void oem_send_connect_state_notify(void *priv, unsigned event)
{
	switch (event)
	{
		case SMD_EVENT_DATA:
			printk("%s data=%d \n",__func__, send_connect_state);
			break;
...
static void oem_send_connect_state(int *state)
{
	int rc = 0;
	...
	if (!is_open)
	{
		rc = smd_open("CONNECT_STATE", &smd_ch, state, oem_send_connect_state_notify);

		if (rc < 0)
		{
			printk("%s open error %d\n",__func__, rc);
		}
		return;
	}

	rc = smd_write(smd_ch, state, sizeof (*state));
...
static void oem_judge_charge_state(void)
{
	do {
		...
		if (POWER_SUPPLY_STATUS_FULL == get_prop_batt_status(the_chip)) {
			...
			wake_unlock(&the_chip->eoc_wake_lock);
			wake_unlock(&oem_charge_wake_lock);
...

		{
			...
			if (charge_state == CHG_STATE_CHG_COMP)
			{
				if (!is_full)
				{
					wake_unlock(&the_chip->eoc_wake_lock);
...
#define VOLTAGE_NOW_THRESH          3500000
#define TRICKLE_CHG_LED_BLINK_TIME  2000

enum oem_batt_led_status {
	OEM_BATT_LED_ST_OFF,
	OEM_BATT_LED_ST_CHG,
	OEM_BATT_LED_ST_FULL,
	OEM_BATT_LED_ST_LOW_BATT,
	OEM_BATT_LED_ST_INVALID
};
static enum oem_batt_led_status oem_batt_led_st = OEM_BATT_LED_ST_INVALID;

void oem_update_charger_task(int led_st)
{
	int code;
	int value = 0;

	if (led_st == OEM_BATT_LED_ST_FULL) {
		debugk("%s(%d):>>oem_notify_power_key_event KEY_SLEEP[%04x]\n", __func__, __LINE__, KEY_SLEEP);
		code = KEY_SLEEP;/* 0x8E */
	}
	else {
		debugk("%s(%d):>>oem_notify_power_key_event KEY_WAKEUP[%04x]\n", __func__, __LINE__, KEY_WAKEUP);
		code = KEY_WAKEUP;/* 0x8F */
	}

	oem_notify_power_key_event(code, value);
}

static void oem_batt_led_control(void)
{
	struct led_control_ex_data led_ctrl;
	enum oem_batt_led_status oem_new_batt_led_st = OEM_BATT_LED_ST_OFF;
	int power_supply_status;
	int power_supply_capacity;
	int voltage_now;
	static int s_update_count = 0;
	static int power_supply_status_last = -1;
	int line;

	if ((OEM_STATUS_LIMIT == oem_charge_status) && (1 == oem_charger_control_flag))
	{
		power_supply_status = oem_last_batt_status;
		line = __LINE__;
	}
	else if (oem_pm8921_bms_is_cyclecorrection_chargeoffstate())
	{
		power_supply_status = oem_bms_last_batt_status;
		line = __LINE__;
	}
	else
	{
		power_supply_status = get_prop_batt_status(the_chip);
		line = __LINE__;
	}

	if (power_supply_status_last != power_supply_status) {
		debugk("%s(%d):power_supply_status[%d->%d] line[%d]\n",
			__func__, __LINE__,
			power_supply_status_last, power_supply_status, line);
		power_supply_status_last = power_supply_status;
	}

	power_supply_capacity = get_prop_batt_capacity(the_chip);
	...
	case POWER_SUPPLY_STATUS_FULL:
		...
		oem_new_batt_led_st = OEM_BATT_LED_ST_FULL;
		atomic_set(&oem_is_full, 1);
		break;
	...
	case POWER_SUPPLY_STATUS_CHARGING:
		if (oem_is_off_charge())
		{
			voltage_now = get_prop_battery_uvolts(the_chip);
			if (VOLTAGE_NOW_THRESH >= voltage_now)
			{
				led_ctrl.color      = LED_COLOR_RED;
				led_ctrl.mode       = LED_BLINK_ON;
				led_ctrl.pattern[0] = TRICKLE_CHG_LED_BLINK_TIME;
				led_ctrl.pattern[1] = TRICKLE_CHG_LED_BLINK_TIME;
				led_ctrl.pattern[2] = 0;
				oem_new_batt_led_st = OEM_BATT_LED_ST_LOW_BATT;
			}
			else
			{
				led_ctrl.color = LED_COLOR_RED;
				oem_new_batt_led_st = OEM_BATT_LED_ST_CHG;
			}
		}
		else
		{
		...
		atomic_set(&oem_is_full, 0);
		break;

	case POWER_SUPPLY_STATUS_NOT_CHARGING:
		led_ctrl.color = LED_COLOR_OFF;
		oem_new_batt_led_st = OEM_BATT_LED_ST_OFF;
		break;

	case POWER_SUPPLY_STATUS_DISCHARGING:
		if (power_supply_capacity < LOW_BATT_CAPACITY)
		{
			led_ctrl.color      = LED_COLOR_RED;
			led_ctrl.mode       = LED_BLINK_ON;
	...
	default:
		/* break; */
		pr_err("power_supply_status = %d\n" ,power_supply_status);
		return;
	}

	++s_update_count;

	if (oem_batt_led_st != oem_new_batt_led_st)
	{
		pm8xxx_set_leddata_ex(&led_ctrl);
		oem_batt_led_st = oem_new_batt_led_st;
		printk("%s led=%d pow=%d cap=%d \n",
		__func__, oem_batt_led_st, power_supply_status, power_supply_capacity);

		if (oem_is_off_charge()) {
			s_update_count = 0;

			debugk("%s(%d):>>oem_update_charger_task st[%d->%d]\n", __func__, __LINE__, oem_batt_led_st, oem_new_batt_led_st);
			oem_update_charger_task(oem_new_batt_led_st);
		}
    }
	else if (s_update_count >= UPDATE_CHARGER_TASK_COUNT) {
		if (oem_is_off_charge()) {
			s_update_count = 0;

			if (oem_new_batt_led_st == OEM_BATT_LED_ST_FULL) {
				debugk("%s(%d):>>oem_update_charger_task st[%d]\n", __func__, __LINE__, oem_new_batt_led_st);
				oem_update_charger_task(oem_new_batt_led_st);
			}
		}
	}
...
static void incall_modem_notify(void *priv, unsigned event)
{
	int len;
	struct incall_status_data	data;
	...
	DEBUG_LOG("incall: event = %u\n", event);

	switch (event)
	{
		case SMD_EVENT_DATA:
			DEBUG_LOG("incall: SMD_EVENT_DATA\n");

			DEBUG_LOG("smd_read_avail: %d\n", smd_read_avail(smd_incall_ch));
			len = smd_read_avail(smd_incall_ch);
			if (len == (sizeof(struct incall_status_data)))
			{
				smd_read_from_cb(smd_incall_ch, &data, sizeof(struct incall_status_data));
				modem_stat.stat_1x = (enum incall_status)data.stat_1x;
				modem_stat.stat_evdo = (enum evdo_status)data.stat_evdo;

				DEBUG_LOG("incall: receive code : 1X=%d, EVDO=%d\n", (int)modem_stat.stat_1x, (int)modem_stat.stat_evdo);
			}
			else if (len > 0)		/* if receive size is invalid, then read it all */
			{
				char buf[10];
				while (smd_read_avail(smd_incall_ch) > 0)
				{
					smd_read_from_cb(smd_incall_ch, &buf, 10);
				}
			}
			DEBUG_LOG("smd_read_left: %d\n", smd_read_avail(smd_incall_ch));
			break;
		case SMD_EVENT_OPEN:
			DEBUG_LOG("incall: smd opened\n");
			smd_incall_opend = true;
			break;
		case SMD_EVENT_CLOSE:
			DEBUG_LOG("incall smd closed\n");
			break;
		case SMD_EVENT_STATUS:
			DEBUG_LOG("incall: smd status\n");
			break;
		case SMD_EVENT_REOPEN_READY:
			DEBUG_LOG("incall: smd reopen ready\n");
			break;
	}
}

static void incall_start(void)
{
	int rc;
	int count;
	...
	smd_incall_opend = false;


	rc = smd_open(INCALL_SMD_PORT_NAME, &smd_incall_ch, NULL, incall_modem_notify);
	if (rc < 0)
	{
		smd_incall_ch = NULL;
		pr_err("smd_open() open error (retry after %d ms)\n", INCALL_OPEN_RETRY_DELAY);
	}
	else
	{
		for (count = 0; count < INCALL_SMD_RETRY_COUNT; ++count)
		{
			if (smd_incall_opend)
				break;
			mdelay(INCALL_SMD_RETRY_DELAY);
		}
		if (count >= INCALL_SMD_RETRY_COUNT)
		{
			smd_close(smd_incall_ch);
			smd_incall_ch = NULL;
	...
	if (!smd_incall_ch)
	{
		DEBUG_LOG("incall_init(): set retry after %d ms\n", INCALL_OPEN_RETRY_DELAY);

		schedule_delayed_work(&incall_timer, msecs_to_jiffies(INCALL_OPEN_RETRY_DELAY));
		return;
	}
}

static void incall_queue_timer_cb(struct work_struct *work)
{
	DEBUG_LOG("%s: INCALL init retry process\n", __func__);

	incall_start();
}

static void oem_remote_notify_init(void)
{
	modem_stat.stat_1x = INCALL_STAT_INVALID;
	modem_stat.stat_evdo = EVDO_STAT_INVALID;

	incall_start();
}

struct modem_status oem_remote_notify_get_modem_status(void)
{
	return modem_stat;
}
...
*/
	...
	oem_get_fact_option();
//	oem_uim_smem_init();
	if (oem_param_share.test_mode_chg_disable == 1)
	{
		charging_disabled = 1;
		printk("%s charging_disabled \n", __func__);
	}
#endif
	chip->dev = &pdev->dev;
#ifdef QUALCOMM_ORIGINAL_FEATURE
...
//	chip->keep_btm_on_suspend = pdata->keep_btm_on_suspend;
	...
	rconn_mohm = pdata->rconn_mohm;/* synchronize chip->rconn_mohm and rconn_mohm */
	chip->led_src_config = pdata->led_src_config;
#ifdef QUALCOMM_ORIGINAL_FEATURE
	chip->has_dc_supply = pdata->has_dc_supply;
#else
	chip->has_dc_supply = 0;
#endif
...
*/
	...
	debugk("%s(%d):off_chg[%d]\n", __func__, __LINE__, oem_is_off_charge());
	pr_info("OK\n");
#endif

	oem_remote_notify_init();
