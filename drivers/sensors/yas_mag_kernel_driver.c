...
#include <linux/miscdevice.h>
#define SENSOR_IO_NAME "geomagnetic_io"
...
static long msg_ioctl(struct file *file_type, 
...
#endif
		if (data->event_output) 
			YLOGD(("[YAS_MAG_EVENT] ABS_RAW_THRESHOLD value = %ld \n",value));
	}
...
#endif
		if (data->event_output) 
			YLOGD(("[YAS_MAG_EVENT] ABS_RAW_DISTORTION dis1 = %d, dis2 = %d, dis3 = %d \n",
...
#endif
		if (data->event_output) 
			YLOGD(("[YAS_MAG_EVENT] ABS_RAW_SHAPE value = %ld \n",value));
	}
...
#endif
	if (data->event_output) 
		YLOGD(("[YAS_MAG_EVENT] ABS_RAW_MODE value = %ld \n",value));
...
static ssize_t geomagnetic_raw_data_show(struct device *dev,struct device_attribute *attr, char *buf)
{
...
static ssize_t geomagnetic_api_event_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	struct input_dev *input_raw = to_input_dev(dev);
	struct geomagnetic_data *data = input_get_drvdata(input_raw);
	int ret = 0;
...
static DEVICE_ATTR(raw_data, S_IRUGO,
	geomagnetic_raw_data_show,
	NULL);
static DEVICE_ATTR(ope_device, S_IWUSR,
	NULL,
	geomagnetic_ope_device_store);
static DEVICE_ATTR(api_event, S_IRUGO,
	geomagnetic_api_event_show,
	NULL);
...
#endif
		if (data->event_output) 
			YLOGD(("[YAS_MAG_EVENT] ABS_RAW_REPORT value = %d \n",value));
	}
...
#endif
		if (data->event_output) 
			YLOGD(("[YAS_MAG_EVENT] ABS_STATUS value = %d \n",accuracy));
...
geomagnetic_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	data->distortion[1] = 15;
	data->distortion[2] = 10;
