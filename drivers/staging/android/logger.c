...
void set_kcj_restart_mode( int mode )
{
	ram_log_info_type *pPanicInfo = (ram_log_info_type *)ADDR_CONTROL_INFO;
    ram_console_type *RamConsoleInfo = (ram_console_type *)MSM_KCJLOG_BASE;

	if ( mode == 1 )
	{
		pPanicInfo->restart_mode = RESTART_MODE_USER;
	}
	else
	{
		pPanicInfo->restart_mode = 0;
	}

	if ( 0 == strncmp((const char *)RamConsoleInfo->header.magic, "DBGC", strlen("DBGC") ) )
	{
		pPanicInfo->console_magic = RAM_CONSOLE_MAGIC;
	}
}
