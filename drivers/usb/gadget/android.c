		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (config == NULL || config->common == NULL
		...
		return -EINVAL;
	if (sscanf(buf, "%2x%s", (unsigned int*)&config->common->vendor_data,
		&config->common->vendor_data[2]) != 2)
		return -EINVAL;
...
static void android_boot_work(struct work_struct *data)
{
	...
	cdev->desc.idProduct = device_desc.idProduct = 0x05a8;
...
android_setup(struct usb_gadget *gadget, const struct usb_ctrlrequest *c)
{
	...
	list_for_each_entry(conf, &dev->configs, list_item)
		if ((&conf->usb_config == cdev->config)||(cdev->config == NULL))
