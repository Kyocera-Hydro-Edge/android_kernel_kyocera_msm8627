...
#define USB_SUSPEND_WAKE_LOCK_TIMEOUT_TIME	(msecs_to_jiffies(OEM_HKADC_MONITOR_TIME_1S + 1000))
	...
	if (pdata->otg_control == OTG_PHY_CONTROL) {
		...
		ulpi_write(phy, OTG_COMP_DISABLE,
			ULPI_SET(ULPI_PWR_CLK_MNG_REG));
                /* Extended Register */
                ulpi_write(phy, 0x39, 0x81); // PARAMETER_OVERRIDE_B
                ulpi_write(phy, 0x3F, 0x82); // PARAMETER_OVERRIDE_C
                ulpi_write(phy, 0x33, 0x83); // PARAMETER_OVERRIDE_D
		/* Enable PMIC pull-up */
...
static int msm_otg_suspend(struct msm_otg *motg)
{
	...
	wake_lock_timeout(&motg->wlock, USB_SUSPEND_WAKE_LOCK_TIMEOUT_TIME);
...
static int msm_otg_notify_chg_type(struct msm_otg *motg)
{
	...
	if (motg->chg_type == USB_SDP_CHARGER)
		tmp_charger_type = POWER_SUPPLY_TYPE_USB;
	else if (motg->chg_type == USB_CDP_CHARGER)
		tmp_charger_type = POWER_SUPPLY_TYPE_USB_CDP;
	else if (motg->chg_type == USB_DCP_CHARGER ||
			motg->chg_type == USB_PROPRIETARY_CHARGER)
		tmp_charger_type = POWER_SUPPLY_TYPE_USB_DCP;
	else if ((motg->chg_type == USB_ACA_DOCK_CHARGER ||
		motg->chg_type == USB_ACA_A_CHARGER ||
		motg->chg_type == USB_ACA_B_CHARGER ||
		motg->chg_type == USB_ACA_C_CHARGER))
		tmp_charger_type = POWER_SUPPLY_TYPE_USB_ACA;
	else
		tmp_charger_type = POWER_SUPPLY_TYPE_BATTERY;

	if (charger_type == tmp_charger_type)
		return 0;
	else
		charger_type = tmp_charger_type;
