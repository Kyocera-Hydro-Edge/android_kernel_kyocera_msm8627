...
#if 1
	wait_for_completion_timeout(&vsync_cntrl.vsync_wait, 1);
#else
	...
	char cont_splashScreen)
{
	...
	mdp_clk_rate = mdp_max_clk;
	printk("%s()MDP CLK RATE[%d]!\n", __func__, mdp_clk_rate);
...
{
	if(lcd_off_charge_flg && oem_panel_power_on_starting){
		printk("[DEV LCD]%s() NO MDP SUSPEND!! \n", __func__);
		return;
	}
