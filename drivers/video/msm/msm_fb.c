 ...
 */
#if 1
unsigned long backlight_duration = 7;
#else
...
struct dsi_buf dm_dsi_buff_tp;
struct dsi_buf dm_dsi_buff_rp;

struct local_disp_state_type local_state;
extern void mdp_refresh_screen_init(void);
struct fb_info *oem_save_fbi = NULL;
extern int bl_que_flag;
int oem_panel_power_on_starting = FALSE;
int kernel_disp_on_flg = 1;
			    ...
			    boolean op_enable)
{
	...
	case FB_BLANK_UNBLANK:
		if (!mfd->panel_power_on) {
			oem_panel_power_on_starting = TRUE;
			msleep(16);
...
*/
	...
	default:
		if (mfd->panel_power_on) {
			...
			mfd->op_enable = TRUE;
			oem_panel_power_on_starting = FALSE;
		}
		break;
	}
...
#else
	var->height = OEM_PHYS_LCD_HEIGHT,
	var->width  = OEM_PHYS_LCD_WIDTH,
#endif
...
#define LCDBL_INIT_Lv  102
void dev_lcd_load_image(struct fb_info *info)
{
  ...
  kernel_disp_raw_bitmap(0, 0, 480, 800, kernel_startup_logo, info->screen_base);
  kernel_disp_raw_bitmap(0, 0, 480, 800, kernel_startup_logo, info->screen_base);
  printk("%s(size[%d])\n",__func__, (unsigned int)info->fix.smem_len);
}

void oem_kernel_init_disp_on( void )
{
  kernel_disp_on_flg = 1;
  msm_fb_open(oem_save_fbi, 0);
  kernel_disp_on_flg = 0;

  if(lcd_off_charge_flg){
    memset(oem_save_fbi->screen_base, 0x00, 480 * 800 * 4);
  }else{
    dev_lcd_load_image(oem_save_fbi);
  }
  msm_fb_pan_display(&(oem_save_fbi->var), oem_save_fbi);
  ...
  if(lcd_off_charge_flg){
    ...
    oem_lcd_backlight_ctrl(60);
    msleep(10);
    oem_lcd_backlight_ctrl(LCDBL_INIT_Lv);
  }
}
...
static void bl_workqueue_handler(struct work_struct *work)
{
	...
	if((pdata) &&(!bl_updated)){
		...
		bl_que_flag = 1;
		oem_lcd_backlight_ctrl(unset_bl_level);
			...
			unsigned long arg)
{
    ...
    case MSMFB_CRC_GET:
        MSM_FB_DEBUG("DISP MSMFB_CRC_GET S\n");

        memset(&dm_dsi_cmds, 0x00, sizeof(dm_dsi_cmds));
        mipi_reg = 0x5;
        dm_dsi_cmds.dtype   = DTYPE_DCS_READ;
        dm_dsi_cmds.last    = 1;
        dm_dsi_cmds.vc      = 0;
        dm_dsi_cmds.ack     = 1;
        dm_dsi_cmds.wait    = 0;
        ...
        crc_data[2] = 0;
        crc_data[3] = (char)(local_state.crc_error_count);
    ...
    case MSMFB_CRC_RESET:
        ...
        local_state.crc_error_count = 0x00000000;

        break;

    case MSMFB_MIPI_REG_WRITE:
        printk(KERN_ERR "MSMFB_MIPI_REG_WRITE:\n");
        ret = copy_from_user(&mipi_reg_data, argp, sizeof(mipi_reg_data));
        if (ret)
        {
            printk(KERN_ERR "MSMFB_MIPI_REG_WRITE: error 1[%d] \n", ret);
            return ret;
        }
        ...
        {
            printk(KERN_ERR "MSMFB_MIPI_REG_WRITE: error 2[%d] \n", mipi_reg_data.type);
            return -EINVAL;
        }
        
        memset(&dm_dsi_cmds, 0x00, sizeof(dm_dsi_cmds));
        dm_dsi_cmds.dtype = mipi_reg_data.type;
        dm_dsi_cmds.last = 1;
        dm_dsi_cmds.vc = 0;
        dm_dsi_cmds.ack = 0;
        dm_dsi_cmds.wait = mipi_reg_data.wait;
        dm_dsi_cmds.dlen = mipi_reg_data.len;
        dm_dsi_cmds.payload = (char *)mipi_reg_data.data;

        printk(KERN_ERR "dtype=%02X\n",dm_dsi_cmds.dtype);
        printk(KERN_ERR "wait=%02X\n",dm_dsi_cmds.wait);
        printk(KERN_ERR "dlen=%02X\n",dm_dsi_cmds.dlen);
        printk(KERN_ERR "payload=[%02X][%02X][%02X][%02X][%02X]\n",dm_dsi_cmds.payload[0],dm_dsi_cmds.payload[1],dm_dsi_cmds.payload[2],dm_dsi_cmds.payload[3],dm_dsi_cmds.payload[4]);

        mipi_set_tx_power_mode(1);

        ret = mipi_dsi_cmds_tx(&dm_dsi_buff_tp, &dm_dsi_cmds, 1);
        printk(KERN_ERR "mipi_dsi_cmds_tx(%08X,%08X,%08X,%d)\n",(int)mfd,(int)&dm_dsi_buff_tp,(int)&dm_dsi_cmds,1);
    ...
    case MSMFB_MIPI_REG_READ:
        ...
        if (ret)
        {
    		printk(KERN_ERR "MSMFB_MIPI_REG_READ: error 1[%d] \n", ret);
            return ret;
        }
        ...
        {
    		printk(KERN_ERR "MSMFB_MIPI_REG_READ: error 2[%d] \n", mipi_reg_data.type);
            return -EINVAL;
        }
    ...
    case MSMFB_DISPLAY_STANDBY:
    {
        ...
        if (ret){
            printk(KERN_ERR "MSMFB_DISPLAY_STANDBY: error 1[%d] \n", ret);
            return ret;
        }
        ...
        {
        case 0:
            ret = msm_fb_blank(FB_BLANK_UNBLANK  , info);
            break;
        case 1:
            ret = msm_fb_blank(FB_BLANK_POWERDOWN, info);
            break;
        default:
            printk(KERN_ERR "MSMFB_DISPLAY_STANDBY: error 2[%d] \n", display_standby_flag);
            ret = -EINVAL;
        }
    }
        break;
    ...
    case MSMFB_LCD_BACKLIGHT:
    {
        ...
        ret = copy_from_user(&value, argp, sizeof(unsigned int));
        if (ret){
            return ret;
        }
        bl_que_flag = 1;
        oem_lcd_backlight_ctrl(value);
        break;
    }
