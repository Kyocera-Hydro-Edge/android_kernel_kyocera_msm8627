...
#define CFG_SET_SCENE_MODE            55
#define CFG_GET_EXIF_INFO             56
#define CFG_SET_FOCUS_MODE            57
#define CFG_GET_FLASH_INFO            58
#define CFG_GET_AE_AWB_STABLE_INFO    59
#define CFG_SET_FLASH_REG             60
#define CFG_MAX                       61
#else 
#define CFG_MAX                       55
#endif 
...
/*-------------------------------------*/
enum {
	CFG_FUNCTION_BASE_VAL = (CFG_MAX),                    /* = CFG_MAX */
	CFG_SET_MANUAL_EXPOSURE = CFG_FUNCTION_BASE_VAL,
	CFG_START_MANUAL_EXPOSURE,
	CFG_STOP_MANUAL_EXPOSURE,
    CFG_SET_ROI_AF_AREA,
    CFG_SET_ROI_AE_AREA,
    CFG_SET_CANSEL_FOCUS,
	CFG_START_CAF_TAKEPICTURE,
	CFG_FUNCTION_MAX_VAL
};
#undef CFG_MAX
#define CFG_MAX                       (CFG_FUNCTION_MAX_VAL)
#endif 
...
#ifdef FEATURE_KYOCERA_MCAM
typedef enum {
  MSM_CAMERA_AEC_FRAME_AVERAGE,
  MSM_CAMERA_AEC_CENTER_WEIGHTED,
  MSM_CAMERA_AEC_SPOT_METERING,
  MSM_CAMERA_AEC_SMART_METERING,
  MSM_CAMERA_AEC_USER_METERING,
  MSM_CAMERA_AEC_MAX_MODES
} msm_camera_auto_exposure_mode_type;

typedef enum {
  MSM_CAMERA_BESTSHOT_OFF = 0,
  MSM_CAMERA_BESTSHOT_AUTO = 1,
  MSM_CAMERA_BESTSHOT_LANDSCAPE = 2,
  MSM_CAMERA_BESTSHOT_SNOW,
  MSM_CAMERA_BESTSHOT_BEACH,
  MSM_CAMERA_BESTSHOT_SUNSET,
  MSM_CAMERA_BESTSHOT_NIGHT,
  MSM_CAMERA_BESTSHOT_PORTRAIT,
  MSM_CAMERA_BESTSHOT_BACKLIGHT,
  MSM_CAMERA_BESTSHOT_SPORTS,
  MSM_CAMERA_BESTSHOT_ANTISHAKE,
  MSM_CAMERA_BESTSHOT_FLOWERS,
  MSM_CAMERA_BESTSHOT_CANDLELIGHT,
  MSM_CAMERA_BESTSHOT_FIREWORKS,
  MSM_CAMERA_BESTSHOT_PARTY,
  MSM_CAMERA_BESTSHOT_NIGHT_PORTRAIT,
  MSM_CAMERA_BESTSHOT_THEATRE,
  MSM_CAMERA_BESTSHOT_ACTION,
  MSM_CAMERA_BESTSHOT_AR,
  MSM_CAMERA_BESTSHOT_MAX
} msm_camera_bestshot_mode_type;

typedef enum {
  MSM_AF_MODE_UNCHANGED = -1,
  MSM_AF_MODE_NORMAL    = 0,
  MSM_AF_MODE_MACRO,
  MSM_AF_MODE_AUTO,
  MSM_AF_MODE_CAF,
  MSM_AF_MODE_INFINITY,
  MSM_AF_MODE_MAX
} msm_isp3a_af_mode_t;

typedef enum {
  MSM_FLASH_REG_PRESTART = 0,
  MSM_FLASH_REG_PREEND,
  MSM_FLASH_REG_START,
  MSM_FLASH_REG_END,
  MSM_FLASH_REG_AEAWB_LOCK_ON,
  MSM_FLASH_REG_AEAWB_LOCK_OFF,
  MSM_FLASH_REG_MAX
} msm_flash_reg_mode_type;
#endif 
...
#ifdef FEATURE_KYOCERA_MCAM
struct exif_info_sensor_t {
    uint32_t  exposure_time;
    uint16_t  iso_speed;
    uint32_t  is_flash_fired;
    uint32_t  reg_exposure;
    uint16_t  reg_ana_gain;
    uint16_t  reg_dig_gain;
};

struct flash_info_sensor_t {
    uint16_t  is_flash_fired;
};

struct ae_awb_stable_info_sensor_t {
    uint16_t  ae;
    uint16_t  awb;
};
#endif
...
/* ROI AF data */
/* ---------------------------- */
#define MAX_ROI_AF_WINDOW_INFO_SENSOR (2)
struct  roi_af_windowinfo_sensor_t {
  uint16_t x;
  uint16_t y;
  uint16_t dx;
  uint16_t dy;
};
struct roi_af_area_info_sensor_t {
    struct roi_af_windowinfo_sensor_t window_area[MAX_ROI_AF_WINDOW_INFO_SENSOR];
};

/* ---------------------------- */
/* ROI AE data */
/* ---------------------------- */
struct  roi_ae_area_sensor_t {
    uint64_t ae_map;
};
struct roi_ae_area_info_sensor_t {
    uint32_t ae_area_num;
    struct roi_ae_area_sensor_t ae_area;
};
...
#ifdef FEATURE_KYOCERA_MCAM
		int32_t scene_mode;
		struct exif_info_sensor_t *exif_info_sensor;
		int32_t focus_mode;
		struct flash_info_sensor_t *flash_info_sensor;
		struct ae_awb_stable_info_sensor_t *ae_awb_stable_info_sensor;
		uint32_t flash_reg_mode;
#endif 
#ifdef FEATURE_KYOCERA_MCAM
		struct manual_exp_cfg manual_exp;
#endif 
#ifdef FEATURE_KYOCERA_MCAM
		struct roi_af_area_info_sensor_t roi_af_area_info;
		struct roi_ae_area_info_sensor_t roi_ae_area_info;
#endif 
	} cfg;
};
