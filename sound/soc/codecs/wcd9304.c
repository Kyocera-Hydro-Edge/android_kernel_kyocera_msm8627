...
enum hs_plug_type {
	HS_PLUG_TYPE_NONE = 0,
	HS_PLUG_TYPE_HEADSET,
	HS_PLUG_TYPE_HEADPHONE,
};

static int hs_jack_type = HS_PLUG_TYPE_NONE;
module_param(hs_jack_type, int, S_IRUGO );
MODULE_PARM_DESC(hs_jack_type, "jack_type=0:NONE/1:HEADSET/2:HEADPHONE)");

 #ifdef DEBUG
static int SITAR_HS_DET_READ_CNT =4;
static int SITAR_HS_PLUG_TYPE_CNT =4;
static int SITAR_HS_SW_PRESS_CNT =2;
static int SITAR_HS_POLLING_TIMER =50;

module_param(SITAR_HS_DET_READ_CNT, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(SITAR_HS_DET_READ_CNT, "SITAR_HS_DET_READ_CNT default:4)");

module_param(SITAR_HS_PLUG_TYPE_CNT, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(SITAR_HS_PLUG_TYPE_CNT, "SITAR_HS_PLUG_TYPE_CNT (default:4)");

module_param(SITAR_HS_SW_PRESS_CNT, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(SITAR_HS_SW_PRESS_CNT, "SITAR_HS_SW_PRESS_CNT (default:2)");

module_param(SITAR_HS_POLLING_TIMER, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(SITAR_HS_POLLING_TIMER, "SITAR_HS_POLLING_TIMER (default:2)");

 #else
#define SITAR_HS_DET_READ_CNT 4
#define SITAR_HS_PLUG_TYPE_CNT 4
#define SITAR_HS_SW_PRESS_CNT 2
#define SITAR_HS_POLLING_TIMER 50
 #endif

enum sitar_hs_det_state {
	HS_DET_STATE_REMOVE,
	HS_DET_STATE_INSERT
};

struct sitar_hs_det_proc_state {
	struct sitar_hs_det_proc_state *next;
	void (*state_enter)(struct snd_soc_codec *);
	void (*state_exit)(struct snd_soc_codec *);
	void (*on_gpio_irq)(struct snd_soc_codec *);
	void (*on_ear_sw_irq)(struct snd_soc_codec *);
	void (*on_ptt_sw_irq)(struct snd_soc_codec *);
...
struct sitar_priv {
...
#ifdef CONFIG_KYOCERA_MSND
	struct sitar_hs_det_proc_state *hs_det_proc_state;
	...
	enum sitar_hs_det_state hs_det_state;
	bool hs_btn_press_flg;

	bool hs_det_timer_active;

	char hs_det_gpio_read_cnt;
	char hs_sw_gpio_read_cnt;
	bool sw_irq_flg;

	bool hs_det_polling_ear_det;
	bool hs_det_polling_ear_sw;

	bool hs_det_gpio_change_status;
	bool hs_det_ear_sw_change_status;

	enum snd_jack_types jack_type;
	bool micbias1_en;
	bool ldo_h_en_dapm;
	bool cfilt2_en_dapm;
	bool micb1_ctl_en_dapm;

 #ifdef CONFIG_KYOCERA_MSND_PTT
	bool hs_det_polling_ptt_sw;
	bool hs_det_ptt_sw_change_status;
	char hs_ptt_sw_gpio_read_cnt;
	bool hs_ptt_sw_press_flg;
 #endif
#endif
};
...
#ifdef CONFIG_KYOCERA_MSND
			if(micb_cfilt_reg == SITAR_A_MICB_CFILT_2_CTL){
				SITAR_ACQUIRE_LOCK(sitar->codec_resource_lock);
				sitar->cfilt2_en_dapm=true;
...
{
#ifdef CONFIG_KYOCERA_MSND
	pr_debug("********** %s(%d) vddio_switch=%d(do nothing)\n",__func__,__LINE__,vddio_switch);
	return;
#else
	struct sitar_priv *sitar = snd_soc_codec_get_drvdata(codec);
	int cfilt_k_val;
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
 	...
 	case SND_SOC_DAPM_PRE_PMU:
		SITAR_ACQUIRE_LOCK(sitar->codec_resource_lock);
		sitar->ldo_h_en_dapm=true;
		SITAR_RELEASE_LOCK(sitar->codec_resource_lock);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		SITAR_ACQUIRE_LOCK(sitar->codec_resource_lock);
		sitar->ldo_h_en_dapm=false;
		SITAR_RELEASE_LOCK(sitar->codec_resource_lock);
		break;
#endif
	case SND_SOC_DAPM_POST_PMU:
...
static const struct snd_soc_dapm_widget sitar_dapm_widgets[] = {
	...
	SND_SOC_DAPM_SUPPLY("LDO_H", SITAR_A_LDO_H_MODE_1, 7, 0,
		sitar_codec_enable_ldo_h, SND_SOC_DAPM_POST_PMU|SND_SOC_DAPM_PRE_PMU|SND_SOC_DAPM_PRE_PMD),
#else
	SND_SOC_DAPM_SUPPLY("LDO_H", SITAR_A_LDO_H_MODE_1, 7, 0,
...
#ifdef CONFIG_KYOCERA_MSND
	if (sitar->micbias1_en) {
		if (reg == SITAR_A_LDO_H_MODE_1){
			pr_debug("**** Overrite LDO_H **** 0x%04x=0x%2x\n",reg,(u8)value);
			value |= 0x80;
		}
		if (reg == SITAR_A_MICB_CFILT_2_CTL){
			pr_debug("**** Overrite CFILT **** 0x%04x=0x%02x\n",reg,(u8)value);
		    value |= 0x80;

		}
		if (reg == SITAR_A_MICB_1_CTL) {
			   pr_debug("**** Overrite MICB1 **** 0x%04x=0x%02x\n",reg,(u8)value);
		       value |= 0x80;
		}
...
static int sitar_codec_remove(struct snd_soc_codec *codec)
{
	...
	mutex_destroy(&sitar->codec_resource_lock);
#ifdef CONFIG_KYOCERA_MSND
	cancel_work_sync(&sitar->hs_det_proc_work);
#endif
	kfree(sitar);
	return 0;
}
...
#ifdef CONFIG_KYOCERA_MSND
static struct sitar_hs_det_proc_state* sitar_set_hs_det_proc_state(struct snd_soc_codec *codec, struct sitar_hs_det_proc_state *state);
static void sitar_jack_detect_push_state(struct snd_soc_codec *codec, struct sitar_hs_det_proc_state *state);
static struct sitar_hs_det_proc_state* sitar_jack_detect_pop_state(struct snd_soc_codec *codec);
static irqreturn_t sitar_jack_detect_on_gpio_irq(int irq, void *data);
static void sitar_jack_detect_on_timer(struct snd_soc_codec *codec);
static void sitar_hs_det_proc_state_init_enter(struct snd_soc_codec *codec);
static void sitar_hs_det_proc_state_polling_enter(struct snd_soc_codec *codec);
static void sitar_hs_det_proc_state_polling_on_timeout(struct snd_soc_codec *codec);
static void sitar_hs_det_proc_state_remove_enter(struct snd_soc_codec *codec);
static void sitar_hs_det_proc_state_remove_on_gpio_irq(struct snd_soc_codec *codec);
static void sitar_hs_det_proc_state_insert_enter(struct snd_soc_codec *codec);
static void sitar_hs_det_proc_state_insert_exit(struct snd_soc_codec *codec);
static void sitar_hs_det_proc_state_insert_on_gpio_irq(struct snd_soc_codec *codec);
static void sitar_hs_btn_det_proc_state_init_on_gpio_irq(struct snd_soc_codec *codec);
static void sitar_hs_btn_det_proc_state_init_on_timeout(struct snd_soc_codec *codec);
static void sitar_hs_btn_det_proc_state_3pinouts_enter(struct snd_soc_codec *codec);
static void sitar_hs_btn_det_proc_state_3pinouts_on_gpio_irq(struct snd_soc_codec *codec);
static void sitar_hs_btn_det_proc_state_4pinouts_enter(struct snd_soc_codec *codec);
static void sitar_hs_btn_det_proc_state_4pinouts_on_gpio_irq(struct snd_soc_codec *codec);
static void sitar_hs_btn_det_proc_state_4pinouts_on_timeout(struct snd_soc_codec *codec);
static void sitar_hs_det_set_timer(struct snd_soc_codec *codec);
static enum hrtimer_restart sitar_hs_det_timer_cb(struct hrtimer *hrtimer);

static irqreturn_t sitar_jack_detect_on_gpio_ear_sw_irq(int irq, void *data);
 #ifdef CONFIG_KYOCERA_MSND_PTT
static irqreturn_t sitar_jack_detect_on_gpio_ptt_sw_irq(int irq, void *data);
 #endif

static void sitar_hs_det_proc_work(struct work_struct *work);
static int sitar_hs_sw_gpio_get_value(struct snd_soc_codec *codec);
 #ifdef CONFIG_KYOCERA_MSND_PTT
static int sitar_hs_ptt_sw_gpio_get_value(struct snd_soc_codec *codec);
 #endif
static void sitar_hs_polling_off(struct snd_soc_codec *codec);
static void __sitar_codec_enable_micbias1(struct snd_soc_codec *codec,bool enable);

static struct sitar_hs_det_proc_state hs_det_proc_state_init =
{
	.state_enter	= sitar_hs_det_proc_state_init_enter,
	.state_exit		= NULL,
	.on_gpio_irq	= NULL,
	.on_timeout		= NULL,
};

static struct sitar_hs_det_proc_state hs_det_proc_state_polling =
{
	.state_enter	= sitar_hs_det_proc_state_polling_enter,
	.state_exit		= NULL,
	.on_gpio_irq	= NULL,
	.on_timeout		= sitar_hs_det_proc_state_polling_on_timeout,
};

static struct sitar_hs_det_proc_state hs_det_proc_state_remove =
{
	.state_enter	= sitar_hs_det_proc_state_remove_enter,
	.state_exit		= NULL,
	.on_gpio_irq	= sitar_hs_det_proc_state_remove_on_gpio_irq,
	.on_timeout		= NULL,
};

static struct sitar_hs_det_proc_state hs_det_proc_state_insert =
{
	.state_enter	= sitar_hs_det_proc_state_insert_enter,
	.state_exit		= sitar_hs_det_proc_state_insert_exit,
	.on_gpio_irq	= sitar_hs_det_proc_state_insert_on_gpio_irq,
	.on_timeout		= NULL,
};

static struct sitar_hs_det_proc_state hs_btn_det_proc_state_init =
{
	.state_enter	= NULL,
	.state_exit		= NULL,
	.on_gpio_irq	= sitar_hs_btn_det_proc_state_init_on_gpio_irq,
	.on_timeout		= sitar_hs_btn_det_proc_state_init_on_timeout,
};

static struct sitar_hs_det_proc_state hs_btn_det_proc_state_3pinouts =
{
	.state_enter	= sitar_hs_btn_det_proc_state_3pinouts_enter,
	.state_exit		= NULL,
	.on_gpio_irq	= sitar_hs_btn_det_proc_state_3pinouts_on_gpio_irq,
	.on_timeout		= NULL,
};

static struct sitar_hs_det_proc_state hs_btn_det_proc_state_4pinouts =
{
	.state_enter	= sitar_hs_btn_det_proc_state_4pinouts_enter,
	.state_exit		= NULL,
	.on_gpio_irq	= sitar_hs_btn_det_proc_state_4pinouts_on_gpio_irq,
	.on_timeout		= sitar_hs_btn_det_proc_state_4pinouts_on_timeout,
};

int sitar_kc_hs_detect(struct snd_soc_codec *codec,
		    const struct sitar_mbhc_config *cfg)
{
	struct sitar_priv *sitar;
	int rc = 0;
	...
	sitar->hs_det_proc_state = NULL;

	rc = request_threaded_irq(sitar->mbhc_cfg.gpio_irq, NULL,
								sitar_jack_detect_on_gpio_irq,
									(IRQF_TRIGGER_RISING |
									 IRQF_TRIGGER_FALLING),
									"sitar-gpio", codec);
	...
	if (!IS_ERR_VALUE(rc)) {
		rc = enable_irq_wake(sitar->mbhc_cfg.gpio_irq);
		if (rc < 0) {
			pr_err("enable_irq_wake %d failed: %d\n", sitar->mbhc_cfg.gpio_irq, rc);
		}
		sitar_set_hs_det_proc_state(codec, &hs_det_proc_state_init);
	} else {
		pr_err("request_threaded_irq %d failed: %d\n", sitar->mbhc_cfg.gpio_irq, rc);
	}

	rc = request_threaded_irq(gpio_to_irq(SITAR_HS_SW_GPIO),NULL,
								sitar_jack_detect_on_gpio_ear_sw_irq,
									(IRQF_TRIGGER_RISING |
									 IRQF_TRIGGER_FALLING),
									"sitar-gpio_ear_sw", codec);

	if (IS_ERR_VALUE(rc)) {
		pr_err("request_threaded_irq %d failed: %d\n", gpio_to_irq(SITAR_HS_SW_GPIO), rc);
	}

 #ifdef CONFIG_KYOCERA_MSND_PTT
	rc = request_threaded_irq(gpio_to_irq(SITAR_HS_PTT_SW_GPIO),NULL,
								sitar_jack_detect_on_gpio_ptt_sw_irq,
									(IRQF_TRIGGER_RISING |
									 IRQF_TRIGGER_FALLING),
									"sitar-gpio_ptt_sw", codec);

	if (IS_ERR_VALUE(rc)) {

		pr_err("request_threaded_irq %d failed: %d\n", gpio_to_irq(SITAR_HS_PTT_SW_GPIO), rc);
	}
 #endif
...
EXPORT_SYMBOL_GPL(sitar_kc_hs_detect);

static struct sitar_hs_det_proc_state* sitar_set_hs_det_proc_state(
				struct snd_soc_codec *codec, struct sitar_hs_det_proc_state *state)
{
	struct sitar_priv *sitar = snd_soc_codec_get_drvdata(codec);
	struct sitar_hs_det_proc_state *last = sitar->hs_det_proc_state;

	sitar->hs_det_proc_state = state;
...
static void sitar_jack_detect_push_state(struct snd_soc_codec *codec,
				struct sitar_hs_det_proc_state *state)
{
	struct sitar_priv *sitar = snd_soc_codec_get_drvdata(codec);

	if(state != NULL) {
		state->next = sitar->hs_det_proc_state;
		sitar->hs_det_proc_state = state;
...
{
	struct sitar_priv *sitar = snd_soc_codec_get_drvdata(codec);
	struct sitar_hs_det_proc_state *head = sitar->hs_det_proc_state;

	if(head != NULL)
	{
		sitar->hs_det_proc_state = head->next;
...
static irqreturn_t sitar_jack_detect_on_gpio_irq(int irq, void *data)
{
    ...
    if (sitar->mbhc_cfg.gpio_irq != irq)
    {
        return IRQ_NONE;
    }
	...
	for(state = sitar->hs_det_proc_state; state != NULL; state = next) {
		next = state->next;
...
static irqreturn_t sitar_jack_detect_on_gpio_ear_sw_irq(int irq, void *data)
{
	...
	if (gpio_to_irq(SITAR_HS_SW_GPIO) != irq)
	{
	  return IRQ_NONE;
	}

	pr_debug("*** %s *** (GPIO status=%d)\n",__func__,sitar_hs_sw_gpio_get_value(codec));

	if(sitar->hs_det_ear_sw_change_status){
		return	IRQ_HANDLED;
	}

	SITAR_ACQUIRE_LOCK(sitar->codec_resource_lock);

	state = sitar->hs_det_proc_state;
	if(state->on_ear_sw_irq != NULL)
		state->on_ear_sw_irq(codec);

	sitar->hs_det_ear_sw_change_status=true;
	sitar->hs_det_polling_ear_sw=true;
	sitar_hs_det_set_timer(codec);
...
static irqreturn_t sitar_jack_detect_on_gpio_ptt_sw_irq(int irq, void *data)
{
	...
	if (gpio_to_irq(SITAR_HS_PTT_SW_GPIO) != irq)
	{
		return IRQ_NONE;
	}

	if (sitar->hs_det_ptt_sw_change_status)
	{
		return IRQ_HANDLED;
	}
	...
	state = sitar->hs_det_proc_state;
	if(state->on_ptt_sw_irq != NULL)
		state->on_ptt_sw_irq(codec);

	sitar->hs_det_ptt_sw_change_status=true;
	sitar->hs_det_polling_ptt_sw=true;
	sitar_hs_det_set_timer(codec);
...
static void sitar_jack_detect_on_timer(struct snd_soc_codec *codec)
{
	...
	SITAR_ACQUIRE_LOCK(sitar->codec_resource_lock);
	sitar->hs_det_timer_active= false;
...
static void sitar_hs_det_proc_state_init_enter(struct snd_soc_codec *codec)
{
    ...
    sitar->hs_det_gpio_read_cnt	= 0;
    sitar->hs_sw_gpio_read_cnt	= 0;
    sitar->hs_btn_press_flg		= false;
    sitar->hs_det_state			= HS_DET_STATE_REMOVE;
 #ifdef CONFIG_KYOCERA_MSND_PTT
	sitar->hs_ptt_sw_gpio_read_cnt	= 0;
	sitar->hs_ptt_sw_press_flg		= false;
 #endif

	hrtimer_init(&sitar->hs_det_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	sitar->hs_det_timer.function = sitar_hs_det_timer_cb;

	INIT_WORK(&sitar->hs_det_proc_work,sitar_hs_det_proc_work);

	SITAR_ACQUIRE_LOCK(sitar->codec_resource_lock);
	sitar_set_hs_det_proc_state(codec, &hs_det_proc_state_polling);
	SITAR_RELEASE_LOCK(sitar->codec_resource_lock);
}
...
{
	struct sitar_priv *sitar = snd_soc_codec_get_drvdata(codec);
	sitar->hs_det_polling_ear_det=true;
	sitar_hs_det_set_timer(codec);
}
...
{
	struct sitar_priv *sitar = snd_soc_codec_get_drvdata(codec);
    static enum sitar_hs_det_state	 last_state = HS_DET_STATE_REMOVE;
    enum sitar_hs_det_state	 next_state;
    ...
    {
		sitar->hs_det_gpio_read_cnt++;
		pr_debug("*** %s *** (state=%d sw_press count=%d)\n",__func__,next_state,sitar->hs_det_gpio_read_cnt);
    }
    else
    {
		last_state = next_state;
		sitar->hs_det_gpio_read_cnt = 1;
		pr_debug("*** %s *** (state=%d sw_press count=%d)\n",__func__,next_state,sitar->hs_det_gpio_read_cnt);
	}

    if(sitar->hs_det_gpio_read_cnt == SITAR_HS_DET_READ_CNT )
	{
		if(next_state == HS_DET_STATE_INSERT)
		{
			pr_debug("*** %s *** (INSERT)\n",__func__);
			sitar_set_hs_det_proc_state(codec, &hs_det_proc_state_insert);
		}
		else
		{
			pr_debug("*** %s *** (REMOVE)\n",__func__);
			sitar_set_hs_det_proc_state(codec, &hs_det_proc_state_remove);
		}

		sitar->hs_det_gpio_read_cnt = 0;
		sitar->hs_det_polling_ear_det=false;
	}
	else
	{
		sitar_hs_det_set_timer(codec);
...
static void sitar_hs_det_proc_state_remove_enter(struct snd_soc_codec *codec)
{
	...
	if(sitar->hs_det_state == HS_DET_STATE_REMOVE)
	{
		sitar_hs_polling_off(codec);
		return;
	}

	if(sitar->jack_type == SND_JACK_HEADSET){
		rc = disable_irq_wake(gpio_to_irq(SITAR_HS_SW_GPIO));
		if (rc < 0) {
			pr_err("enable_irq_wake %d failed: %d\n", gpio_to_irq(SITAR_HS_SW_GPIO), rc);
		}
 #ifdef CONFIG_KYOCERA_MSND_PTT
		rc = disable_irq_wake(gpio_to_irq(SITAR_HS_PTT_SW_GPIO));
		if (rc < 0) {
			pr_err("enable_irq_wake %d failed: %d\n", gpio_to_irq(SITAR_HS_PTT_SW_GPIO), rc);
		}
 #endif
	...
	sitar->jack_type =0;
	hs_jack_type = HS_PLUG_TYPE_NONE;

	key_dm_driver_set_port(0x12);
	key_dm_driver_set_port(0x10);

	snd_soc_jack_report_no_dapm(sitar->mbhc_cfg.headset_jack, 0, SITAR_JACK_MASK);
	snd_soc_jack_report_no_dapm(sitar->mbhc_cfg.button_jack, 0, SND_JACK_BTN_0);
...
static void __sitar_codec_enable_micbias1(struct snd_soc_codec *codec,bool enable)
{
	...
	pr_debug("%s enable=%d => %d\n", __func__, sitar->micbias1_en ,enable);
	if(sitar->micbias1_en==enable){
		return;
	}

	if(enable)
	{
		sitar->micbias1_en=true;
		if (!sitar->mclk_enabled) {
			sitar_codec_disable_clock_block(codec);
			sitar_codec_enable_bandgap(codec, SITAR_BANDGAP_MBHC_MODE);
			sitar_codec_enable_clock_block(codec, 1);
		...
		snd_soc_update_bits(codec, SITAR_A_LDO_H_MODE_1    ,0x80, 0x80);
		snd_soc_update_bits(codec, SITAR_A_MICB_CFILT_2_CTL,0x80, 0x80);
		snd_soc_update_bits(codec, SITAR_A_MICB_1_CTL      ,0x80, 0x80);
	}else{
		sitar->micbias1_en=false;

		
		if(!sitar->micb1_ctl_en_dapm)
			snd_soc_update_bits(codec, SITAR_A_MICB_1_CTL      ,0x80, 0x00);
		if(!sitar->cfilt2_en_dapm)
			snd_soc_update_bits(codec, SITAR_A_MICB_CFILT_2_CTL,0x80, 0x00);
		if(!sitar->ldo_h_en_dapm)
			snd_soc_update_bits(codec, SITAR_A_LDO_H_MODE_1    ,0x80, 0x00);
...
static void sitar_hs_det_proc_state_insert_enter(struct snd_soc_codec *codec)
{
	...
	__sitar_codec_enable_micbias1(codec,true);

	sitar_jack_detect_push_state(codec, &hs_btn_det_proc_state_init);

	sitar->hs_det_gpio_change_status=true;
	sitar_hs_det_set_timer(codec);
...
static void sitar_hs_det_proc_state_insert_exit(struct snd_soc_codec *codec)
{
	...
	sitar->hs_det_timer_active = false;
	sitar->hs_det_gpio_change_status=false;
	sitar->hs_det_ear_sw_change_status=false;
 #ifdef CONFIG_KYOCERA_MSND_PTT
	sitar->hs_det_ptt_sw_change_status=false;
 #endif
	hrtimer_cancel(&sitar->hs_det_timer);
}
...
static void sitar_hs_btn_det_proc_state_init_on_gpio_irq(struct snd_soc_codec *codec)
{
	struct sitar_priv *sitar = snd_soc_codec_get_drvdata(codec);
	sitar->hs_det_gpio_change_status=true;
	sitar_jack_detect_pop_state(codec);
}
...
static void sitar_hs_btn_det_proc_state_init_on_timeout(struct snd_soc_codec *codec)
{
	...
	if(sitar->jack_type){
		sitar->hs_sw_gpio_read_cnt = 0;
		sitar->hs_det_polling_ear_sw=false;

 #ifdef CONFIG_KYOCERA_MSND_PTT
		sitar->hs_det_polling_ptt_sw=false;
 #endif
		switch(sitar->jack_type){
			case	SND_JACK_HEADPHONE:
				sitar_set_hs_det_proc_state(codec, &hs_btn_det_proc_state_3pinouts);
				break;
			case	SND_JACK_HEADSET:
				sitar_set_hs_det_proc_state(codec, &hs_btn_det_proc_state_4pinouts);
				break;
			default:
				break;
		}
	...
	next_value = sitar_hs_sw_gpio_get_value(codec);

	if(last_value == next_value)
	{
		sitar->hs_sw_gpio_read_cnt++;
	}
	else
	{
		last_value = next_value;
		sitar->hs_sw_gpio_read_cnt = 1;
	}

	pr_debug("** %s(%d) status=%d count=%d\n",__func__,__LINE__,next_value,sitar->hs_sw_gpio_read_cnt);

	if(sitar->hs_sw_gpio_read_cnt == SITAR_HS_PLUG_TYPE_CNT)
	{
		if(next_value)
		{
			sitar_set_hs_det_proc_state(codec, &hs_btn_det_proc_state_3pinouts);
...
static void sitar_hs_btn_det_proc_state_3pinouts_enter(struct snd_soc_codec *codec)
{
	...
	pr_debug("** %s(%d) lastjacktype=%d\n",__func__,__LINE__,sitar->jack_type);

	if(sitar->hs_det_state==HS_DET_STATE_INSERT){
		pr_debug("** %s(%d) ALREADY INSERT STATE **\n",__func__,__LINE__);
		
		return;
	}
	hs_jack_type = HS_PLUG_TYPE_HEADPHONE;

	key_dm_driver_set_port(0xC0);

	snd_soc_jack_report_no_dapm(sitar->mbhc_cfg.headset_jack, SND_JACK_HEADPHONE, SITAR_JACK_MASK);

	sitar->hs_det_state = HS_DET_STATE_INSERT;
	sitar->jack_type = SND_JACK_HEADPHONE;
...
static void sitar_hs_btn_det_proc_state_4pinouts_enter(struct snd_soc_codec *codec)
{
	...
	hs_jack_type = HS_PLUG_TYPE_HEADSET;

	key_dm_driver_set_port(0xC0);

	snd_soc_jack_report_no_dapm(sitar->mbhc_cfg.headset_jack, SND_JACK_HEADSET, SITAR_JACK_MASK);

	sitar->hs_det_state = HS_DET_STATE_INSERT;
	sitar->jack_type = SND_JACK_HEADSET;


	sitar->hs_det_polling_ear_sw=true;
	rc = enable_irq_wake(gpio_to_irq(SITAR_HS_SW_GPIO));
	if (rc < 0) {
		pr_err("enable_irq_wake %d failed: %d\n", gpio_to_irq(SITAR_HS_SW_GPIO), rc);
	}
 ...
 #ifdef CONFIG_KYOCERA_MSND_PTT
	sitar->hs_det_polling_ptt_sw=true;
	rc = enable_irq_wake(gpio_to_irq(SITAR_HS_PTT_SW_GPIO));
	if (rc < 0) {
		pr_err("enable_irq_wake %d failed: %d\n", gpio_to_irq(SITAR_HS_PTT_SW_GPIO), rc);
	}
 #endif
...
static void sitar_hs_btn_det_proc_state_4pinouts_on_gpio_irq(struct snd_soc_codec *codec)
{
	pr_debug("** %s(%d) pop state \n",__func__,__LINE__);
	sitar_jack_detect_pop_state(codec);
}


static void sitar_hs_btn_det_proc_state_4pinouts_on_timeout_ear_sw_det(struct snd_soc_codec *codec)
{
    static int last_value = 0;
    int next_value;
	struct sitar_priv *sitar = snd_soc_codec_get_drvdata(codec);
	...
	if(!sitar->hs_det_polling_ear_sw){
	...
	{
		sitar->hs_sw_gpio_read_cnt++;
		pr_debug("** %s(%d) polling status=%d / count=%d \n",__func__,__LINE__,last_value,sitar->hs_sw_gpio_read_cnt);
	}
	else
	{
		last_value = next_value;
	...
	if(sitar->hs_sw_gpio_read_cnt == SITAR_HS_SW_PRESS_CNT)
	{
		if(next_value){
			if(sitar->hs_btn_press_flg == false)
			{
				key_dm_driver_set_port(0x11);

				snd_soc_jack_report_no_dapm(sitar->mbhc_cfg.button_jack, SND_JACK_BTN_0, SND_JACK_BTN_0);
				sitar->hs_btn_press_flg = true;
			}
		}else{
			if(sitar->hs_btn_press_flg == true){
				key_dm_driver_set_port(0x10);
...
static void sitar_hs_btn_det_proc_state_4pinouts_on_timeout_ptt_sw_det(struct snd_soc_codec *codec)
{
    static int last_value = 0;
    int next_value;
	struct sitar_priv *sitar = snd_soc_codec_get_drvdata(codec);
	...
	if(!sitar->hs_det_polling_ptt_sw){
		
		return;
	}

	next_value = !sitar_hs_ptt_sw_gpio_get_value(codec);

	if(last_value == next_value){
		sitar->hs_ptt_sw_gpio_read_cnt++;
		pr_debug("** %s(%d) polling status=%d / count=%d \n",__func__,__LINE__,last_value,sitar->hs_ptt_sw_gpio_read_cnt);
	}else{
		last_value = next_value;
		sitar->hs_ptt_sw_gpio_read_cnt = 1;
		pr_debug("** %s(%d) polling status=%d / count=%d \n",__func__,__LINE__,last_value,sitar->hs_ptt_sw_gpio_read_cnt);
	}

	if(sitar->hs_ptt_sw_gpio_read_cnt == SITAR_HS_SW_PRESS_CNT)
	{
		if(next_value){
			if(sitar->hs_ptt_sw_press_flg == false)
			{
				key_dm_driver_set_port(0x11);

				snd_soc_jack_report_no_dapm(sitar->mbhc_cfg.button_jack, SND_JACK_BTN_0, SND_JACK_BTN_0);
				sitar->hs_ptt_sw_press_flg = true;
			}
		}else{
			if(sitar->hs_ptt_sw_press_flg == true){
				key_dm_driver_set_port(0x10);
...
static void sitar_hs_btn_det_proc_state_4pinouts_on_timeout(struct snd_soc_codec *codec)
{
	...
	sitar_hs_btn_det_proc_state_4pinouts_on_timeout_ear_sw_det(codec);
 #ifdef CONFIG_KYOCERA_MSND_PTT
	sitar_hs_btn_det_proc_state_4pinouts_on_timeout_ptt_sw_det(codec);
 #endif


	polling_status = sitar->hs_det_polling_ear_det | sitar->hs_det_polling_ear_sw <<4;
 #ifdef CONFIG_KYOCERA_MSND_PTT
	polling_status |= sitar->hs_det_polling_ptt_sw<<8;
 #endif

	if(polling_status != 0){
		pr_debug("** %s(%d) Polling continue (%03x)\n",__func__,__LINE__,polling_status);
		sitar_hs_det_set_timer(codec);
	}else{
		pr_debug("** %s(%d) Polling stop   \n",__func__,__LINE__);
	}
}
...
static void sitar_hs_det_set_timer(struct snd_soc_codec *codec)
{
	...
	if(sitar->hs_det_timer_active == true){
		pr_debug("** %s(%d) Already working Timer**\n",__func__,__LINE__);
		return;
	}

	debounce_time = ktime_set(0, SITAR_HS_POLLING_TIMER*1000*1000);	
	sitar->hs_det_timer_active=true;
	hrtimer_start(&sitar->hs_det_timer, debounce_time, HRTIMER_MODE_REL);
}
...
static enum hrtimer_restart sitar_hs_det_timer_cb(struct hrtimer *hrtimer)
{
	...
	sitar = container_of(hrtimer, struct sitar_priv, hs_det_timer);

    schedule_work(&sitar->hs_det_proc_work);
...
static void sitar_hs_det_proc_work(struct work_struct *work)
{
	...
	sitar = container_of(work, struct sitar_priv, hs_det_proc_work);
	codec = sitar->codec;

	sitar_jack_detect_on_timer(codec);
}

static int sitar_hs_sw_gpio_get_value(struct snd_soc_codec *codec)
{
	int	val=0;
	val=gpio_get_value_cansleep(SITAR_HS_SW_GPIO);
	return val;
}
...
{
	int	val=0;
	val=gpio_get_value_cansleep(SITAR_HS_PTT_SW_GPIO);
	return val;
}
 #endif
...
static void sitar_hs_polling_off(struct snd_soc_codec *codec)
{
	...
	__sitar_codec_enable_micbias1(codec,false);
	sitar->hs_det_gpio_change_status=false;
	sitar->hs_det_ear_sw_change_status=false;
 #ifdef CONFIG_KYOCERA_MSND_PTT
	sitar->hs_det_ptt_sw_change_status=false;
