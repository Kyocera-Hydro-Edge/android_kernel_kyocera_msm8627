...
#ifdef CONFIG_KYOCERA_MSND
#define MSM8930_RCV_ON 1
#define MSM8930_RCV_OFF 0
#endif
...
#ifdef CONFIG_KYOCERA_MSND
#define SPKR_BOOST_GPIO  PM8921_GPIO_PM_TO_SYS(10)
#define LEFT_SPKR_AMPL_GPIO  SPKR_BOOST_GPIO
#define SPK_PAMP_5VCNT_GPIO  79
#define RCV_AMP_GPIO  PM8921_GPIO_PM_TO_SYS(11)
#else
...
#ifdef CONFIG_KYOCERA_MSND
#define MSM8930_RCVAMP_POS    0x1
#define MSM8930_RCVAMP_NEG    0x2
#endif
...
#define HS_SW_GPIO_CFG \
   GPIO_CFG(SITAR_HS_SW_GPIO, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA)
 #ifdef CONFIG_KYOCERA_MSND_PTT
#define HS_SW_PTT_GPIO_CFG \
   GPIO_CFG(SITAR_HS_PTT_SW_GPIO, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA)
 #endif
#endif
...
#ifdef CONFIG_KYOCERA_MSND
static int msm8930_rcv_control;
...
#ifdef CONFIG_KYOCERA_MSND
static void msm8930_enable_rcv_amp_gpio(void)
{
    ...
    pr_debug("%s: msm8930_enable_rcv_amp_gpio() \n", __func__);
    ret = gpio_request(RCV_AMP_GPIO, "PIEZO_RCV_AMP");
    if (ret) {
        pr_err("%s: Error requesting Receiver Amp GPIO %u\n",
        __func__, RCV_AMP_GPIO);
        msm8930_rcv_pamp = 0;
        return;
    }

    pr_debug("%s: enable Receiver Amp gpio\n", __func__);
    gpio_direction_output(RCV_AMP_GPIO, 1);

    adapter = i2c_get_adapter(2);
    if (!adapter) {
        pr_err("%s: invalid i2c adapter\n", __func__);
        gpio_direction_output(RCV_AMP_GPIO, 0);
        gpio_free(RCV_AMP_GPIO);
        msm8930_rcv_pamp = 0;
        return;
    }
...
static void msm8930_disable_rcv_amp_gpio(void)
{
    unsigned char wBuff[16];
    struct i2c_adapter *adapter;
    int res;
    ...
    pr_debug("%s: msm8930_disable_rcv_amp_gpio() \n", __func__);
		...
		if ((msm8930_ext_spk_pamp & SPK_AMP_POS) &&
			...
			if (ret) {
				pr_err("%s: Failed to config ampl gpio %u\n",
					__func__, SPK_PAMP_5VCNT_GPIO);
				return;
			}
			pr_debug("%s: sleeping 50 ms after turning on "
				" external Top Speaker Ampl\n", __func__);
			usleep_range(50000, 50000);

			ret = msm8930_cfg_spkr_gpio(
					LEFT_SPKR_AMPL_GPIO,
					1, "LEFT_SPKR_AMPL_GPIO");
			if (ret) {
				pr_err("%s: Failed to config ampl gpio %u\n",
			...
			pr_debug("%s: slepping 6 ms after turning on external "
				" Left Speaker Ampl\n", __func__);
...
#ifdef CONFIG_KYOCERA_MSND
		pr_debug("%s: sleeping 100 ms before turning off external Top"
			" Spkaker Ampl\n", __func__);
		usleep_range(100000, 100000);

		pr_debug("%s: Free speaker amp gpio %u\n",
				__func__, SPKR_BOOST_GPIO);
		gpio_direction_output(LEFT_SPKR_AMPL_GPIO, 0);
		gpio_free(LEFT_SPKR_AMPL_GPIO);
		msm8930_ext_spk_pamp = 0;
		pr_debug("%s: slepping 80 ms after turning on external "
			" Left Speaker Ampl\n", __func__);
		usleep_range(80000, 80000);
		gpio_direction_output(SPK_PAMP_5VCNT_GPIO, 0);
		gpio_free(SPK_PAMP_5VCNT_GPIO);
		msm8930_ext_spk_pamp = 0;
#else
		if (socinfo_get_pmic_model() == PMIC_MODEL_PM8917) {
			gpio_free(LEFT_SPKR_AMPL_GPIO);
...
#ifdef CONFIG_KYOCERA_MSND
static void msm8930_ext_rcv_control(struct snd_soc_codec *codec)
{
	struct snd_soc_dapm_context *dapm = &codec->dapm;

	
	pr_debug("%s: rcv_control = %d\n", __func__, msm8930_rcv_control);
	return;

	mutex_lock(&dapm->codec->mutex);

	pr_debug("%s: rcv_control = %d\n", __func__, msm8930_rcv_control);
	if (msm8930_rcv_control == MSM8930_RCV_ON) {
...
static void msm8930_rcv_power_amp_on(u32 rcv)
{
    if (rcv & (MSM8930_RCVAMP_POS | MSM8930_RCVAMP_NEG)) {

        if ((msm8930_rcv_pamp & MSM8930_RCVAMP_POS) &&
            (msm8930_rcv_pamp & MSM8930_RCVAMP_NEG) ) {
            pr_debug("%s() Receiver Amp already "
                        "turned on. rcv = 0x%08x\n", __func__, rcv);
            return;
        }
        msm8930_rcv_pamp |= rcv;
        if ((msm8930_rcv_pamp & MSM8930_RCVAMP_POS) &&
            (msm8930_rcv_pamp & MSM8930_RCVAMP_NEG) ) {

	        msm8930_enable_rcv_amp_gpio();
        }
    } else {
...
static void msm8930_rcv_power_amp_off(u32 rcv)
{
    if (rcv & (MSM8930_RCVAMP_POS | MSM8930_RCVAMP_NEG)) {
		...
		msm8930_disable_rcv_amp_gpio();
        msm8930_rcv_pamp = 0;

		pr_debug("%s: sleeping 4 ms after turning off Receiver Amp\n", __func__);
...
static int msm8930_get_rcvamp(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol)
{
    pr_debug("%s: msm8930_rcv_control = %d\n", __func__, msm8930_rcv_control);
    ucontrol->value.integer.value[0] = msm8930_rcv_control;
    return 0;
}
static int msm8930_set_rcvamp(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol)
{
    struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);

	pr_debug("%s()\n", __func__);
    if (msm8930_rcv_control == ucontrol->value.integer.value[0])
        return 0;

    msm8930_rcv_control = ucontrol->value.integer.value[0];
    msm8930_ext_rcv_control(codec);
    return 1;
}

static int msm8930_rcvramp_event (struct snd_soc_dapm_widget *w,
    struct snd_kcontrol *k, int event)
{
	pr_debug("%s() %x\n", __func__, SND_SOC_DAPM_EVENT_ON(event));
    if (SND_SOC_DAPM_EVENT_ON(event)) {
        if (!strncmp(w->name, "Ext Rcv Pos", 11)) {
            msm8930_rcv_power_amp_on(MSM8930_RCVAMP_POS);
        } else if (!strncmp(w->name, "Ext Rcv Neg", 11)) {
            msm8930_rcv_power_amp_on(MSM8930_RCVAMP_NEG);
        } else {
        ...
        if (!strncmp(w->name, "Ext Rcv Pos", 11)) {
            msm8930_rcv_power_amp_off(MSM8930_RCVAMP_POS);
        } else if (!strncmp(w->name, "Ext Rcv Neg", 11)) {
            msm8930_rcv_power_amp_off(MSM8930_RCVAMP_NEG);
        } else {
...
#ifdef CONFIG_KYOCERA_MSND
static const char *spk_rcv_switch[] = {"Off", "On"};

static const struct soc_enum spk_rcv_enum =
	SOC_ENUM_SINGLE_EXT(2, spk_rcv_switch);

static const struct snd_kcontrol_new spk1_sw =
	SOC_DAPM_ENUM_VIRT("LINEOUT1_SPK", spk_rcv_enum);
static const struct snd_kcontrol_new spk2_sw =
	SOC_DAPM_ENUM_VIRT("LINEOUT2_SPK", spk_rcv_enum);
static const struct snd_kcontrol_new rcv1_sw =
	SOC_DAPM_ENUM_VIRT("LINEOUT1_RCV", spk_rcv_enum);
static const struct snd_kcontrol_new rcv2_sw =
	SOC_DAPM_ENUM_VIRT("LINEOUT2_RCV", spk_rcv_enum);
...
static const struct snd_soc_dapm_widget msm8930_dapm_widgets[] = {
...
#ifdef CONFIG_KYOCERA_MSND
	SND_SOC_DAPM_SPK("Ext Rcv Pos", msm8930_rcvramp_event),
	SND_SOC_DAPM_SPK("Ext Rcv Neg", msm8930_rcvramp_event),
#endif
	...
	SND_SOC_DAPM_VIRT_MUX("LINEOUT1_SPK", SND_SOC_NOPM, 1, 0, &spk1_sw),
	SND_SOC_DAPM_VIRT_MUX("LINEOUT2_SPK", SND_SOC_NOPM, 1, 0, &spk2_sw),
	SND_SOC_DAPM_VIRT_MUX("LINEOUT1_RCV", SND_SOC_NOPM, 1, 0, &rcv1_sw),
	SND_SOC_DAPM_VIRT_MUX("LINEOUT2_RCV", SND_SOC_NOPM, 1, 0, &rcv2_sw),
#else
...
static const struct snd_soc_dapm_route common_audio_map[] = {
	...
	{"Ext Spk Left Pos", NULL, "LINEOUT1_SPK"},
	{"Ext Spk Left Neg", NULL, "LINEOUT2_SPK"},
	{"LINEOUT1_SPK", "On", "LINEOUT1 PA"},
	{"LINEOUT2_SPK", "On", "LINEOUT2 PA"},


	{"Ext Rcv Pos", NULL, "LINEOUT1_RCV"},
	{"Ext Rcv Neg", NULL, "LINEOUT2_RCV"},
	{"LINEOUT1_RCV", "On", "LINEOUT1 PA"},
	{"LINEOUT2_RCV", "On", "LINEOUT2 PA"},

	
	{"Ext HAC", NULL, "EAR PA"},

	
	{"AMIC2", NULL, "MIC BIAS1 External"},
	{"MIC BIAS1 External", NULL, "Headset Mic"},

	
	{"AMIC1", NULL, "MIC BIAS2 External"},
	{"MIC BIAS2 External", NULL, "Handset Main Mic"},

	{"AMIC3", NULL, "MIC BIAS2 External"},
	{"MIC BIAS2 External", NULL, "Handset Sub Mic"},
#else
	/* Speaker path */
...
#ifdef CONFIG_KYOCERA_MSND
	SOC_ENUM_EXT("Receiver Function", msm8930_enum[3], msm8930_get_rcvamp,
		msm8930_set_rcvamp),
#endif
...
static int msm8930_audrx_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	snd_soc_dapm_enable_pin(dapm, "Ext Rcv Pos");
	snd_soc_dapm_enable_pin(dapm, "Ext Rcv Neg");
	snd_soc_dapm_enable_pin(dapm, "Ext HAC");
	snd_soc_dapm_enable_pin(dapm, "Handset Main Mic");
	snd_soc_dapm_enable_pin(dapm, "Handset Sub Mic");
#endif
	...
	err = gpio_request(SITAR_HS_SW_GPIO, "HEADSET_SW");
	if (err < 0) {
		pr_err("%s: gpio_request %d failed %d\n", __func__,
		       SITAR_HS_SW_GPIO, err);
 ...
 #ifdef CONFIG_KYOCERA_MSND_PTT
	err = gpio_request(SITAR_HS_PTT_SW_GPIO, "HEADSET_PTT_SW");
	if (err < 0) {
		pr_err("%s: gpio_request %d failed %d\n", __func__,
		       SITAR_HS_PTT_SW_GPIO, err);
		gpio_free(mbhc_cfg.gpio);
		gpio_free(SITAR_HS_SW_GPIO);
		return err;
	}
	gpio_tlmm_config( HS_SW_PTT_GPIO_CFG, GPIO_CFG_ENABLE );
 #endif

	err = sitar_kc_hs_detect(codec, &mbhc_cfg);
	if (err < 0) {
		pr_err("%s: sitar_kc_hs_detect err:%d\n", __func__, err);
		gpio_free(mbhc_cfg.gpio);
		gpio_free(SITAR_HS_SW_GPIO);
 #ifdef CONFIG_KYOCERA_MSND_PTT
		gpio_free(SITAR_HS_PTT_SW_GPIO);
 #endif
		return err;
	}
#else
